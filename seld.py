...
def main(argv):
    ...
    avg_scores_val = []
    avg_scores_test = []
    if params['model_nb'] == 1:
        import keras_model1 as keras_model
        separate_channels = False
    elif params['model_nb'] == 2:
        import keras_model2 as keras_model
        separate_channels = False
    elif params['model_nb'] == 3:
        import keras_model3 as keras_model
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        data_gen_train = cls_data_generator.DataGenerator(
            ...
            seq_len=params['sequence_length'], feat_label_dir=params['feat_label_dir'], channels_separate=separate_channels
        )

        print('Loading validation dataset:')
        data_gen_val = cls_data_generator.DataGenerator(
            dataset=params['dataset'], split=val_splits[split_cnt], batch_size=params['batch_size'],
            seq_len=params['sequence_length'], feat_label_dir=params['feat_label_dir'], shuffle=False, channels_separate=separate_channels
        )
        ...
        os.environ["MKL_NUM_THREADS"] = "23040" 
        os.environ["NUMEXPR_NUM_THREADS"] = "23040" 
        os.environ["OMP_NUM_THREADS"] = "23040"
...
#        model = load_model('D:/seld-dcase2019-master/seld-dcase2019-master/models/10_X_tree_normal_foa_eval_split0_model.h5')
        ...
        for epoch_cnt in range(nb_epoch):
            ...
            hist = model.fit_generator(
                ...
                epochs=5 if epoch_cnt == 0 else params['epochs_per_fit'],
                verbose=2
            )
            ...
            patience_cnt += 1
            if seld_metric[epoch_cnt] <= best_seld_metric or (np.round(seld_metric[epoch_cnt], decimals=3) == np.round(best_seld_metric, decimals=3) and tr_loss[epoch_cnt] < best_tr_loss):
                best_seld_metric = seld_metric[epoch_cnt]
                best_tr_loss = tr_loss[epoch_cnt]
        ...
        data_gen_test = cls_data_generator.DataGenerator(
            ...
            is_eval=True if params['mode'] is 'eval' else False, channels_separate=separate_channels
        )

        print('\nLoading the best model and predicting results on the testing split')
        model = load_model('{}_model.h5'.format(unique_name))
        #model = load_model('D:/seld-dcase2019-master/seld-dcase2019-master/models/12_separable_X_tree_mixed_foa_dev_split1_model.h5')
